...
print("\n" + "=" * 80)
print("HIL BMS Project Readiness Check")
print("=" * 80)

# 1. Check protocol
print("\n[1/5] Checking MCU Protocol...")
try:
    from pc_simulator.communication.protocol_mcu import (
        HILFrameEncoder, HIL_FRAME_HEADER, HIL_FRAME_FOOTER, 
        HIL_FRAME_VERSION, HIL_FRAME_OVERHEAD
    )
    print(f"  [OK] Protocol module loaded")
    print(f"  [OK] Frame header: 0x{HIL_FRAME_HEADER:02X} (expected: 0xAA)")
    print(f"  [OK] Frame footer: 0x{HIL_FRAME_FOOTER:02X} (expected: 0x55)")
    print(f"  [OK] Frame version: 0x{HIL_FRAME_VERSION:02X} (expected: 0x01)")
...
try:
    from pc_simulator.communication.uart_tx_mcu import MCUCompatibleUARTTransmitter
...
try:
    ...
    assert frame[0] == HIL_FRAME_HEADER, "Header mismatch"
    assert frame[1] == HIL_FRAME_VERSION, "Version mismatch"
    assert frame[-1] == HIL_FRAME_FOOTER, "Footer mismatch"
    
    data_length = (frame[2] << 8) | frame[3]
    expected_size = HIL_FRAME_OVERHEAD + data_length
...
try:
    from pc_simulator.main import main
    from pc_simulator.plant.pack_model import BatteryPack16S
    from pc_simulator.afe.wrapper import AFEWrapper
...
print("  3. Run: python pc_simulator/main.py --port COM3 --duration 60 --rate 50")
print("\nFrame format:")
print("  - Header: 0xAA")
print("  - Version: 0x01")
